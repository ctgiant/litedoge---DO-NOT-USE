                           ...
                           list<pair<CTxDestination, int64_t> >& listSent, int64_t& nFee, string& strSentAccount) const
{
    ...
    BOOST_FOREACH(const CTxOut& txout, vout)
    {
	    ...
	    // LDOGENOTE: CoinControl possible fix related... with HD wallet we need to report change?
...
void CWallet::AvailableCoins(vector<COutput>& vCoins, bool fOnlyConfirmed, const CCoinControl *coinControl, AvailableCoinsType coin_type, bool useIX) const
{
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            int nDepth = pcoin->GetDepthInMainChain();
            if (nDepth <= 0) // LDOGENOTE: coincontrol fix / ignore 0 confirm 
                continue;
           ...
           /* for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if(coin_type == ONLY_DENOMINATED) {
                    ...
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if(found && coin_type == ONLY_NONDENOMINATED_NOTMN) found = (pcoin->vout[i].nValue != 5000000*COIN); // do not use MN funds
                } else {
                    found = true;
                }
                if(!found) continue;
...
bool CWallet::SelectCoins(int64_t nTargetValue, unsigned int nSpendTime, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64_t& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    //if we're doing only denominated, we need to round up to the nearest .1LDOGE
    if(coin_type == ONLY_DENOMINATED){
        ...
        BOOST_FOREACH(int64_t v, darkSendDenominations)
        {
            ...
            BOOST_FOREACH(const COutput& out, vCoins)
            {
                if(out.tx->vout[out.i].nValue == v                                            //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1*COIN)+100 //round the amount up to .1LDOGE over
...
bool CWallet::SelectCoinsByDenominations(int nDenom, int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& setCoinsRet, vector<COutput>& setCoinsRet2, int64_t& nValueRet, int nDarksendRoundsMin, int nDarksendRoundsMax)
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(out.tx->vout[out.i].nValue < 10000*COIN && out.tx->vout[out.i].nValue != (1000*COIN)+1000000) continue;
        if(fMasterNode && out.tx->vout[out.i].nValue == 2500000000*COIN) continue; //masternode input
        if(nValueRet + out.tx->vout[out.i].nValue <= nValueMax){
            ...
            if(fFound100000 && fFound10000 && fFound1000 && fFound100 && fFound10 && fFound1 && fFoundDot1){ //if fulfilled
                ...
                if((nDenom & (1 << 0)) && out.tx->vout[out.i].nValue ==      ((1000000000*COIN)    +1000000000000)) {fAccepted = true;}
                else if((nDenom & (1 << 1)) && out.tx->vout[out.i].nValue == ((100000000*COIN)     +100000000000)) {fAccepted = true;}
                else if((nDenom & (1 << 2)) && out.tx->vout[out.i].nValue == ((10000000*COIN)      +10000000000)) {fAccepted = true;}
                else if((nDenom & (1 << 3)) && out.tx->vout[out.i].nValue == ((1000000*COIN)       +1000000000)) {fAccepted = true;}
                else if((nDenom & (1 << 4)) && out.tx->vout[out.i].nValue == ((100000*COIN)        +100000000)) {fAccepted = true;}
                else if((nDenom & (1 << 5)) && out.tx->vout[out.i].nValue == ((10000*COIN)         +10000000)) {fAccepted = true;}
                else if((nDenom & (1 << 6)) && out.tx->vout[out.i].nValue == ((1000*COIN)        +1000000)) {fAccepted = true;}
            } else {
                //Criterion has not been satisfied, we will only take 1 of each until it is.
                if((nDenom & (1 << 0)) && out.tx->vout[out.i].nValue ==      ((1000000000*COIN)    +1000000000000)) {fAccepted = true; fFound100000 = true;}
                else if((nDenom & (1 << 1)) && out.tx->vout[out.i].nValue == ((100000000*COIN)     +100000000000)) {fAccepted = true; fFound10000 = true;}
                else if((nDenom & (1 << 1)) && out.tx->vout[out.i].nValue == ((10000000*COIN)      +10000000000)) {fAccepted = true; fFound1000 = true;}
                else if((nDenom & (1 << 1)) && out.tx->vout[out.i].nValue == ((1000000*COIN)       +1000000000)) {fAccepted = true; fFound100 = true;}
                else if((nDenom & (1 << 1)) && out.tx->vout[out.i].nValue == ((100000*COIN)        +100000000)) {fAccepted = true; fFound10 = true;}
                else if((nDenom & (1 << 2)) && out.tx->vout[out.i].nValue == ((10000*COIN)         +10000000)) {fAccepted = true; fFound1 = true;}
                else if((nDenom & (1 << 3)) && out.tx->vout[out.i].nValue == ((1000*COIN)        +1000000)) {fAccepted = true; fFoundDot1 = true;}
            }
...
string CWallet::PrepareDarksendDenominate(int minRounds, int maxRounds)
{
    ...
    */
    ...
    BOOST_REVERSE_FOREACH(int64_t v, vDenoms){
        ...
        bool fAccepted = false;
        if((darkSendPool.sessionDenom & (1 << 0))      && v == ((1000000000*COIN) +1000000000000)) {fAccepted = true;}
        else if((darkSendPool.sessionDenom & (1 << 1)) && v == ((100000000*COIN)  +100000000000)) {fAccepted = true;}
        else if((darkSendPool.sessionDenom & (1 << 2)) && v == ((10000000*COIN)   +10000000000)) {fAccepted = true;}
        else if((darkSendPool.sessionDenom & (1 << 3)) && v == ((1000000*COIN)    +1000000000)) {fAccepted = true;}
        else if((darkSendPool.sessionDenom & (1 << 4)) && v == ((100000*COIN)     +100000000)) {fAccepted = true;}
        else if((darkSendPool.sessionDenom & (1 << 5)) && v == ((10000*COIN)      +10000000)) {fAccepted = true;}
        else if((darkSendPool.sessionDenom & (1 << 6)) && v == ((1000*COIN)     +1000000)) {fAccepted = true;}
        if(!fAccepted) continue;
