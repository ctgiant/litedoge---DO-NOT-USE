    ...
    if(fMasterNode) {
        ...
        BOOST_FOREACH(const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat Litedoge and make it unusable. To
...
bool CDarkSendPool::DoAutomaticDenominating(bool fDryRun, bool ready)
{
    ...
    int64_t nBalanceNeedsAnonymized = nAnonymizeLitedogeAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
//        }
    ...
    if (!pwalletMain->SelectCoinsDark(nValueMin, nBalanceNeedsAnonymized, vCoins, nValueIn, 0, nDarksendRounds))
    {
        ...
        if (pwalletMain->SelectCoinsDark(nValueMin, 99999999999*COIN, vCoins, nValueIn, -2, 0))
        {
    ...
    if(!sessionFoundMasternode){
        ...
        double fLitedogeSubmitted = (sessionTotalValue / CENT);
        LogPrintf("Submitting Darksend for %f DRK CENT - sessionTotalValue %d\n", fLitedogeSubmitted, sessionTotalValue);
...
int CDarkSendPool::GetDenominationsByAmount(int64_t nAmount, int nDenomTarget){
    ...
    BOOST_REVERSE_FOREACH(int64_t v, darkSendDenominations){
        if(nDenomTarget != 0){
            ...
            if((nDenomTarget & (1 << 0)) &&      v == ((1000000000*COIN)+1000000000000)) {fAccepted = true;}
            else if((nDenomTarget & (1 << 1)) && v == ((100000000*COIN) +100000000000)) {fAccepted = true;}
            else if((nDenomTarget & (1 << 2)) && v == ((10000000*COIN)  +10000000000)) {fAccepted = true;}
            else if((nDenomTarget & (1 << 3)) && v == ((1000000*COIN)   +1000000000)) {fAccepted = true;}
            else if((nDenomTarget & (1 << 4)) && v == ((100000*COIN)    +100000000)) {fAccepted = true;}
            else if((nDenomTarget & (1 << 5)) && v == ((10000*COIN)     +10000000)) {fAccepted = true;}
            else if((nDenomTarget & (1 << 6)) && v == ((1000*COIN)      +1000000)) {fAccepted = true;}
...
void ThreadCheckDarkSendPool()
{
    ...
    RenameThread("litedoge-darksend");
        ...
        if(c % 60*(nLiquidityProvider+1) == 0){
            if(nLiquidityProvider!=0){
                ...
                if(nRand == 50+nLiquidityProvider && pwalletMain->GetAverageAnonymizedRounds() > 8){
                    ...
                    if(nLeftToAnon > 9999999) nLeftToAnon = 9999999;
                    nAnonymizeLitedogeAmount = (rand() % nLeftToAnon)+3;
                } else {
