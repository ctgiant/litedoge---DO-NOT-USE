...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeLitedogeAmount"))
        settings.setValue("nAnonymizeLitedogeAmount", 5000000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeLitedogeAmount = settings.value("nAnonymizeLitedogeAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeLitedogeAmount"))
        SoftSetArg("-anonymizelitedogeamount", settings.value("nAnonymizeLitedogeAmount").toString().toStdString());
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeLitedogeAmount:
            nAnonymizeLitedogeAmount = value.toInt();
            settings.setValue("nAnonymizeLitedogeAmount", nAnonymizeLitedogeAmount);
            emit anonymizeLitedogeAmountChanged(nAnonymizeLitedogeAmount);
            break;
#ifdef USE_NATIVE_I2P
        case I2PUseI2POnly:
        {
